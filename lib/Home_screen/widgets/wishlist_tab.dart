    ...
    return BlocBuilder<HomeCubit, HomeState>(
      builder: (context, state) {
        if (state is HomeSuccessState) {
          final wishlistProducts = state.products?.where((p) => state.wishlistItemIds.contains(p.id)).toList() ?? [];
                                ...
                                ElevatedButton(
                                  onPressed: () {
                                    if (product.id != null) {
                                      ...
                                      ScaffoldMessenger.of(context).showSnackBar(
                                        SnackBar(
                                          content: Text(state.cartItemIds.contains(product.id) ? 'Removed from Cart' : 'Added to Cart'),
                                          duration: const Duration(seconds: 1),
                                          backgroundColor: AppColors.mainColor,
                                        ),
                                      );
                                  ...
                                  child: Text(
                                    state.cartItemIds.contains(product.id) ? "Remove" : "Add to Cart",
